...
const EMAILJS_TEMPLATE_ID = "template_rox1c4r";
const EMAILJS_PUBLIC_KEY  = "ye6YG9_SJgUTsO0rA";
...
const ContactItem = ({ icon, label, value, href }) => (
  ...
  >
    ...
    <div>
      ...
      <div style={{ fontSize: "0.95rem", fontWeight: 600, color: "#111110", wordBreak: "break-all" }}>{value}</div>
    </div>
  </a>
);
...
const labelStyle = { fontSize: "0.76rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#7a7a74", display: "flex", gap: "0.2rem", marginBottom: "0.5rem" };
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await window.emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { from_name: form.nom, from_email: form.email, project_name: form.projet, project_type: form.type, message: form.message });
      setStatus("success");
  ...
  const onBlur  = (name) => (e) => { e.target.style.borderColor = errors[name] ? "#c0392b" : "#e5dfd3"; e.target.style.background = errors[name] ? "#fff5f5" : "#faf8f4"; };
  ...
  return (
    <section id="contact" style={{ padding: isMobile ? "4rem 1.2rem" : "7rem 5rem", background: "#faf8f4" }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? "2.5rem" : "5rem", alignItems: "start" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5dfd3", borderRadius: "22px", padding: isMobile ? "1.5rem" : "2.5rem", boxShadow: "0 6px 32px rgba(0,0,0,0.06)" }}>
          <h3 style={{ fontSize: "1.2rem", fontWeight: 700, color: "#111110", marginBottom: "0.3rem" }}>Parlez-nous de votre projet</h3>
          <p style={{ fontSize: "0.82rem", color: "#7a7a74", marginBottom: "1.8rem" }}>Tous les champs sont obligatoires <span style={{ color: "#c0392b" }}>*</span></p>
